...
for beta, lmax, (lamd1, U), ferr in iterlist:
    ...
    mat_equ = expm(- beta * hams) / np.trace(expm(- beta * hams))
    ...
    json_init = {
        ...
        "cfw-data": {
            "forward": True,
            "tau": complex_2_json(tau),
            "alpha": complex_2_json(alpha),
            "tf": complex_2_json(tf),
        },
        "equilibrium": {
            "ti": 0,
            "tf": tf,
    ...
    magic_str = '{}-cfw-dt-filter'.format(ferr)
    ...
    cmd += '&&' + r'/usr/local/bin/jemalloc.sh  ../thermo_fermi_2.out'
    ...
    benchmark('prop-rho-eq1.dat', magic_str, 'fermi')
